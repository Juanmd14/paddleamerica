...
- Accounts (`profiles`) and ranking players (`players`) are separate. `players.profile_id` links them (unique, set null when the account is deleted). Admins link or create the player from `/admin/usuarios/[id]`; `set_profile_category` also updates the linked player's `category`. Admins are named with `set_user_admin` from the same page, never automatically by email (email confirmation is off).
